...
import { getMetaMaskAddress, disconnectMetaMask, isMetaMaskConnected } from '@/lib/metamaskAuth';

export default function Header() {
  const [userAddress, setUserAddress] = useState('');
  const [walletType, setWalletType] = useState<'stacks' | 'metamask' | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      if (userData && userData.profile && userData.profile.stxAddress) {
        ...
        const metamaskAddr = getMetaMaskAddress();
        if (metamaskAddr) {
          setUserAddress(metamaskAddr);
